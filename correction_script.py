...
def find_stars(ra_center, dec_center, size_deg, image, header):
...
def star_check(image, header, gaia_star):
...
def better_psf_fitting(fits_file, header, shift_amount, test_size, gaia_star_field):
...
def plot_data_sim_comparison(masked_x_coords, masked_y_coords, file, header, gaia_star_field, masked_best_x, masked_best_y):
